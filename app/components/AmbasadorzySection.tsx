...
export default function AmbasadorzySection() {
  const ambassadors = [
    {
          ...
          {ambassadors.map((ambassador, index) => (
            <div
              ...
              className={`group relative overflow-hidden rounded-xl backdrop-blur-sm border transition-all duration-300 ${
                ambassador.isMainAmbassador
                  ? "bg-gradient-to-br from-[#d4af37]/20 via-[#f7e199]/10 to-transparent border-[#d4af37]/30 hover:border-[#d4af37]"
                  : "bg-gradient-to-br from-white/10 via-white/5 to-transparent border-white/10 hover:border-[#d4af37]/50"
              }`}
            >
              {/* Badge dla głównej ambasadorki */}
              {ambassador.isMainAmbassador && (
                <div className="absolute top-3 right-3 z-10">
                  <span className="inline-block px-3 py-1 bg-gradient-to-r from-[#d4af37] to-[#f7e199] text-black text-xs font-bold rounded-full">
                    AMBASADORKA
                  </span>
                </div>
              )}
              ...
              <div className="p-6 flex flex-col items-center text-center h-full">
                {/* Zdjęcie */}
                <div
                  className={`relative w-32 h-32 mb-4 rounded-full overflow-hidden ring-2 transition-all ${
                    ambassador.isMainAmbassador
                      ? "ring-[#d4af37] group-hover:ring-[#f7e199]"
                      : "ring-[#d4af37]/30 group-hover:ring-[#d4af37]"
                  }`}
                >
                  <Image
