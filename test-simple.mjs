...
console.log('🚀 Rozpoczynam dodawanie wszystkich produktów...');

async function main() {
  try {
    // Najpierw usuń testowy produkt (opcjonalnie)
    await prisma.product.deleteMany({
      where: {
        title: "Test Morpheus-8"
      }
    });
    console.log('🗑️ Usunięto testowe produkty');

    // TUTAJ WKLEJ SWOJE PRODUKTY Z products.js
    // Zamień export const products = [ na const products = [
    // i usuń export na początku
    const products = [
      // WKLEJ TUTAJ ZAWARTOŚĆ Z products.js
      // Pamiętaj o zmianie slug nazw na małe litery bez polskich znaków
          {
            title: "Morpheus-8",
          slug: "morpheus",
          description: `
    ...
    console.log(`📦 Będę dodawać ${products.length} produktów...`);
    ...
    console.log('🎉 Wszystkie produkty zostały dodane pomyślnie!');
    
    // Sprawdź liczbę produktów
    const count = await prisma.product.count();
    console.log(`📊 Łącznie produktów w bazie: ${count}`);

  } catch (error) {
    console.error('❌ Błąd podczas dodawania produktów:', error.message);
  } finally {
